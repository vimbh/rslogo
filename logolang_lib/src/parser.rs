...
impl Parser {
    ...
    pub fn procedure(&mut self, tokens: &mut VecDeque<Token>) -> Result<AstNode, String> {
        ...
        if start_token.kind != TokenKind::PROCSTART {
            return Err(format!(
                "Expected 'PROCSTART' token, found {:?}",
                start_token.kind
            ));
        }
        ...
        while let Some(token) = tokens.front() {
            ...
            arg_tokens.push(
                tokens
                    .pop_front()
                    .ok_or("Expected an arg/body to PROCNAME")?
                    .value,
            );
        }
        ...
        if end_token.kind != TokenKind::PROCEND {
            return Err(format!(
                "Expected PROCEND token, instead found {:?}",
                end_token.kind
            ));
        }
    ...
    fn while_statement(&mut self, tokens: &mut VecDeque<Token>) -> Result<AstNode, String> {
        ...
        match condition {
            AstNode::BooleanOp { .. } | AstNode::ComparisonOp { .. } | AstNode::IdentRef(_) => {}
            _ => {
                return Err("<EXPR1> in WHILE <EXPR1> [<EXPR2>], must return a boolean".to_string())
            }
        }
        ...
        if l_paren_token.kind != TokenKind::LPAREN {
            return Err(format!(
                "Expected '[' after 'IF' in IF <EXPR1> [<EXPR2>], found {:?}",
                l_paren_token.value
            ));
        }
        ...
        if r_paren_token.kind != TokenKind::RPAREN {
            return Err(format!(
                "Expected ']' after '<EXPR2>' in IF <EXPR1> [<EXPR2>], found {:?}",
                r_paren_token.kind
            ));
        }
    ...
    fn make_op(&mut self, tokens: &mut VecDeque<Token>) -> Result<AstNode, String> {
        ...
        let ident_token = tokens
            .pop_front()
            .ok_or("Expected identifier token after 'Make'")?;
        match ident_token.kind {
            TokenKind::IDENT | TokenKind::IDENTREF => {} // Continue
            _ => {
                return Err(format!(
                    "Expected identifier/reference to identifier token after 'Make', found {:?}",
                    ident_token.kind
                ))
            }
        }
    ...
    fn binary_op(&mut self, tokens: &mut VecDeque<Token>) -> Result<AstNode, String> {
        ...
        if operator_token.kind != TokenKind::BINOP {
            return Err(format!(
                "Expected binary operator token, found {:?}",
                operator_token.kind
            ));
        }
    ...
    fn add_assign(&mut self, tokens: &mut VecDeque<Token>) -> Result<AstNode, String> {
        ...
        if operator_token.kind != TokenKind::ADDASSIGN {
            return Err(format!(
                "Expected AddAssign operator token, found {:?}",
                operator_token.kind
            ));
        }
        ...
        if var.kind != TokenKind::IDENT {
            return Err(format!(
                "Addasign op expected a variable name, instead received: {:?}, of type {:?}",
                var.value, var.kind
            ));
        }
    ...
    fn comparison_op(&mut self, tokens: &mut VecDeque<Token>) -> Result<AstNode, String> {
        ...
        let operator_token = tokens
            .pop_front()
            .ok_or("Expected comparison operator token")?;
    ...
    fn bool_op(&mut self, tokens: &mut VecDeque<Token>) -> Result<AstNode, String> {
        ...
        let operator_token = tokens
            .pop_front()
            .ok_or("Expected boolean operator token")?;
        if operator_token.kind != TokenKind::BOOLOP {
            return Err(format!(
                "Expected boolean operator token, found {:?}",
                operator_token.kind
            ));
        }
    ...
    fn num(&mut self, tokens: &mut VecDeque<Token>) -> Result<AstNode, String> {
        ...
        let num_value = num_token
            .value
            .parse::<f32>()
            .map_err(|_| format!("Invalid number token: {}", num_token.value))?;
